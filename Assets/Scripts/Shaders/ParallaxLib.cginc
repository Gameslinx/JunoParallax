...
struct PositionData
{
    ...
    float3 scale;
    float rot;
    uint index;  //The first index in the triangle indices. index, index + 1 and index + 2 form the triangle
...
// Compare the three normals in each terrain triangle. If they deviate from each other by a significant amount, it means that the gradient of the terrain is changing.
// This can lead to floating rocks (if using clusters of rocks as models), so we can define a tolerance for this "normal deviance".
