	...
	SubShader
	{
		...
		Pass
		{
			...
			float3 pixel_shader(v2f_screenPos i) : SV_TARGET
			{
				...
				color = lerp(color, _LightColor0, _IceTransparency);
				return lerp(color.rgb, newCol.rgb, _IceTransparency);
			}
			ENDCG
		...
		Pass
		{
			...
			float4 pixel_shader(v2f_screenPos_lighting i) : SV_TARGET
			{
				...
				color = lerp(color, _LightColor0, _IceTransparency);
				color = lerp(color, newCol, _IceTransparency);
